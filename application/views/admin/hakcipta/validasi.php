          ...
          <div class="card card-info">
              ...
              <!-- form start -->
                ...
                <div class="form-group mt-3">
                    <label for="commentspencipta">Comments</label>
                    <textarea class="form-control" id="commentsdetail" name="commentsdetail" rows="3"></textarea>
                </div>
                </div>
                <!-- /.card-body -->
            </div>
            ...
            <div class="card card-info">
                ...
                <div class="form-group mt-3">
                    <label for="commentspencipta">Comments</label>
                    <textarea class="form-control" id="commentspencipta" name="commentspencipta" rows="3"></textarea>
                </div>
                </div>
                <!-- /.card-body -->
              </div>
                  ...
                  <div class="card-footer bg-light">
                      <div class="form-group row">
                          <div class="col-md-6 col-sm-6 offset-md-3">
                              <button type="button" id="submit" name="submit" class="btn btn-success">Submit Validasi</button>
                          </div>
                      </div>
                  </div>
              </div>
...
<script>
  $('#submit').click(function(){
    var commentspencipta = $('#commentspencipta').val();
    var commentsdetail = $('#commentsdetail').val();
    if (commentsdetail === "" && commentspencipta === "") {
      Swal.fire({
        title: 'Anda Yakin ?',
        text: "Ingin Menyelesaikan Data Ini",
        icon: 'warning',
        ...
        if (result.value) {
          $.ajax({
            type: "POST",
            data: {id: "<?= $hc['idHakCipta'] ?>"},
            url: "<?php echo site_url('admin/hakcipta/selesaiHakCipta')?>",
            dataType: "JSON",
            success: function (data) {
              ...
              if (data.status == true) {
                Swal.fire({
                  ...
                  if (result.value) {
                  window.location.replace("<?php echo site_url('admin/hakcipta/tablevalidasi') ?>");
                  }
                });
              } else {
      ...
      Swal.fire({
        title: 'Anda Yakin ?',
        text: "Data akan dikembalikan ke User untuk di Revisi",
        icon: 'warning',
        showCancelButton: true,
        confirmButtonColor: '#3085d6',
        cancelButtonColor: '#d33',
        ...
        if (result.value) {
          $.ajax({
            ...
            data: {
              ...
              commentspencipta:commentspencipta
            },
            url: "<?php echo site_url('admin/hakcipta/validasiHakCipta')?>",
            dataType: "JSON",
            success: function (data) {
